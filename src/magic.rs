...
impl Magic {
    ...
    fn genmagic(questions: &Vec<BitBoard>, answers: &Vec<BitBoard>, result_index: usize) -> Option<Magic> {
        ...
        while max_guess > 0 {
            // make a guess
            let guess = Magic::random_bitboard(&mut rng);
    ...
    fn save_magic(piece_type: usize, sq: Square, directions: &Vec<fn(Square) -> Option<Square>>, result_index: usize) -> usize {
        unsafe {
            RAYS[piece_type][sq.to_int() as usize] = Magic::gen_rays(sq, EMPTY, directions, true);
        }

        let questions = Magic::permute(Magic::gen_rays(sq, EMPTY, directions, false));
    
        let answers = questions.iter().map(|bb| Magic::gen_rays(sq, *bb, directions, true)).collect::<Vec<BitBoard>>();
        ...
        unsafe {
            MAGIC_NUMBERS[piece_type][sq.to_int() as usize] = Magic::genmagic(&questions, &answers, result_index).unwrap();
        }
    ...
    fn all_directions() -> Vec<fn(Square) -> Option<Square>> {
        let mut v = Vec::new();
        v.extend(Magic::rook_directions());
        v.extend(Magic::bishop_directions());
        v
    }

    /// Generate the rook magic number for a particular square
    fn gen_rooks(sq: Square, result_index: usize) -> usize {
        let directions: Vec<fn(Square) -> Option<Square>> = Magic::rook_directions();
        Magic::save_magic(ROOK, sq, &directions, result_index)
    }

    /// Generate the bishop magic number for a particular square
    fn gen_bishops(sq: Square, result_index: usize) -> usize {
        let directions: Vec<fn(Square) -> Option<Square>> = Magic::bishop_directions();
        Magic::save_magic(BISHOP, sq, &directions, result_index)
    }

    /// Generate all king moves for a particular square.
    fn gen_king_moves(sq: Square) {
        unsafe {
            KING_MOVES[sq.to_index()] = Magic::all_directions().iter()
                                                               .map(|d| d(sq))
    ...
    fn gen_knight_moves(sq: Square) {
        unsafe {
            KNIGHT_MOVES[sq.to_index()] = Magic::knight_directions().iter()
                                                                    .map(|d| d(sq))
                                                                    .fold(EMPTY, |cur, os| match os {
                                                                        None => cur,
                                                                        Some(sq) => cur | BitBoard::from_square(sq)
    ...
    fn gen_line_and_between(sq1: Square, sq2: Square) {
        let directions: Vec<fn(Square) -> Option<Square>> = Magic::all_directions();
        let mut rays = Magic::get_rook_rays(sq1) | Magic::get_bishop_rays(sq1);
    ...
    pub fn get_pawn_moves(sq: Square, color: Color, blockers: BitBoard) -> BitBoard {
        Magic::get_pawn_attacks(sq, color, blockers) ^ Magic::get_pawn_quiets(sq, color, blockers)
    }
